...
def scores(**overrides: int | None) -> dict[str, int | None]:
    values: dict[str, int | None] = {field: DEFAULT_SCORE for field in SCORE_FIELDS}
    values.update(overrides)
    for key, value in values.items():
        if value is None:
            continue
        if not isinstance(value, int) or value < 1 or value > SCALE_MAX:
            raise ValueError(f"{key} must be 1..{SCALE_MAX} or None, got {value}")
    return values
        ...
        "responses": {
            ...
            104: {
                **scores(
                    ...
                    school_clarity=2,
                    school_question=2,
                    school_class_motivation=2,
                    school_class_satisfaction=2,
                    school_test_motivation=2,
                    school_test_satisfaction=1,
                    school_growth=2,
                    school_test_expectation=1,
        ...
        "responses": {
            ...
            104: {
                **scores(
                    ...
                    home_growth=4,
                    home_continuation=4,
                    school_understanding=1,
                    school_security=2,
                    school_fun=2,
                    school_clarity=1,
                    school_question=2,
                    school_class_motivation=2,
                    school_class_satisfaction=1,
                    school_test_motivation=2,
                    school_test_satisfaction=1,
                    school_growth=1,
                    school_test_expectation=1,
                    school_continuation=2,
                    study_fun=3,
                    study_goal=4,
                    study_improvement_motivation=4,
                ),
...
def write_json(path: Path, rows: list[dict]) -> None:
    ...
    payload = {
        "source": "demo",
        "note": "共有テストデータと同じ列構造のデモ用データ。定量は5段階。実在の生徒情報は含まない。",
