const { isValidScore, previousYearMonth } = require("./overview");

function labelFor(key, metricConfig) {
  for (const group of metricConfig.groups) {
    const found = group.metrics.find((metric) => metric.key === key);
    if (found) {
      return found.label;
    }
  }
  return key;
}

function primaryKeys(metricConfig) {
  return new Set(
    metricConfig.groups.flatMap((group) => group.metrics.map((metric) => metric.key))
  );
}

function scoreValue(row, key, scaleMax) {
  if (!row) {
    return null;
  }
  const value = row[key];
  return isValidScore(value, scaleMax) ? value : null;
}

function deltaOf(current, previous) {
  if (current === null || previous === null) {
    return null;
  }
  const delta = current - previous;
  return delta === 0 ? null : delta;
}

function badgesFor(key, value, previous, rules, scaleMax) {
  const badges = [];

  if (value === null) {
    badges.push("未回答");
    return badges;
  }

  if (rules.attention_keys.includes(key) && value <= rules.attention_max) {
    badges.push(rules.labels.attention);
  } else if (value <= rules.low_score_max) {
    badges.push(`${rules.low_score_max}以下`);
  }

  if (previous !== null && previous - value >= rules.drop_min) {
    badges.push(`先月-${rules.drop_min}`);
  }

  return badges;
}

function textValue(row, key) {
  const value = row[key];
  if (typeof value !== "string" || value.trim() === "") {
    return null;
  }
  return value;
}

function buildStudentDetail(demoData, options) {
  const { metricConfig, questionConfig, rules, studentId, month } = options;
  const scaleMax = metricConfig.scale_max;
  const months = demoData.months.slice();
  const selectedMonth = month || months[months.length - 1];

  if (!months.includes(selectedMonth)) {
    const error = new Error("unknown_month");
    error.code = "unknown_month";
    throw error;
  }

  const id = Number(studentId);
  const current = demoData.responses.find(
    (row) => row.student_id === id && row.year_month === selectedMonth
  );

  if (!current) {
    const error = new Error("unknown_student");
    error.code = "unknown_student";
    throw error;
  }

  const previousMonth = previousYearMonth(selectedMonth);
  const previous = demoData.responses.find(
    (row) => row.student_id === id && row.year_month === previousMonth
  );
  const keys = primaryKeys(metricConfig);

  return {
    student_id: current.student_id,
    student_name: current.student_name,
    grade: current.grade,
    months,
    selected_month: selectedMonth,
    rules_note: rules.ui_note || rules.note,
    groups: questionConfig.groups.map((group) => ({
      id: group.id,
      label: group.label,
      key_metrics: group.questions
        .filter((key) => keys.has(key))
        .map((key) => {
          const value = scoreValue(current, key, scaleMax);
          const prior = scoreValue(previous, key, scaleMax);
          return {
            key,
            label: labelFor(key, metricConfig),
            value,
            delta: deltaOf(value, prior),
            scale: scaleMax
          };
        }),
      alerts: group.questions
        .map((key) => {
          const value = scoreValue(current, key, scaleMax);
          const prior = scoreValue(previous, key, scaleMax);
          return {
            key,
            label: labelFor(key, metricConfig),
            value,
            badges: badgesFor(key, value, prior, rules, scaleMax)
          };
        })
        .filter((item) => item.badges.length > 0),
      questions: group.questions.map((key) => {
        const value = scoreValue(current, key, scaleMax);
        const prior = scoreValue(previous, key, scaleMax);
        return {
          key,
          label: labelFor(key, metricConfig),
          value,
          delta: deltaOf(value, prior),
          scale: scaleMax
        };
      }),
      text_fields: (group.text_fields || []).map((field) => ({
        key: field.key,
        label: field.label,
        value: textValue(current, field.key)
      }))
    }))
  };
}

module.exports = {
  buildStudentDetail
};
